if (view_get_visible(5)) {
	// Assigns the camera to viewport 6
	view_set_camera(6, global.rightarm_camera);

	// Sets the coordinates for the viewport to appear on screen
	var viewport_size = 600
	view_set_wport(6, viewport_size);
	view_set_hport(6, viewport_size);

	// Places viewport box
	view_set_xport(6, (view_get_wport(0) - viewport_size) / 2);
	view_set_yport(6, (view_get_hport(0) - viewport_size) / 2);

	// Focuses the camera on the specific body region
	focus_camera(global.rightarm_camera, 1215, 345, 1425, 700);
